...
namespace Timeliner
{
    internal class TimeBarHandler : MouseHandlerBase<TimelineView>
    {
        ...
        public override void MouseDrag(object sender, PointF arg, PointF delta, int callNr)
        {
            if (Button == 3)
            {
                var closestKeyframe = Instance.Tracks.SelectMany(track => track.KeyframeViews).OrderBy(kf => kf.Model.Time.Value).Where(kf => Math.Abs(Instance.Ruler.TimeToXPos(kf.Model.Time.Value) - arg.X) < 10);
                if (closestKeyframe.Any())
                    Instance.Timer.Time = closestKeyframe.First().Model.Time.Value;
                else
                    Instance.Timer.Time = Instance.Ruler.XPosToTime(arg.X);
            }
            else
                Instance.Timer.Time = Instance.Ruler.XPosToTime(arg.X);
        }
